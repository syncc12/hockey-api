...
def goalies(db,pbgs,allPlayers,game,isAway=True,REPLACE_VALUE=-1):
  ...
  return {
    ...
    f'{homeAway}StartingGoalieCatchesT': str(safe_chain(getPlayer(allPlayers,startingId),0,'shootsCatches',default=0)),
    ...
    f'{homeAway}BackupGoalieCatchesT': str(safe_chain(getPlayer(allPlayers,backupId),0,'shootsCatches',default=0)),
...
def goalie(db,playerId,allPlayers,game,isStarting=True,isAway=True,REPLACE_VALUE=-1):
  ...
  return {
    ...
    f'{homeAway}{startingBackup}GoalieCatchesT': str(safe_chain(getPlayer(allPlayers,playerId),0,'shootsCatches',default=0)),
