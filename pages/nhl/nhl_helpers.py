...
def ai(db, game_data, useProjectedLineup, models):
  ...
  if not data['isProjectedLineup'] and not useProjectedLineup:
    ...
    for i in data_keys:
      ...
      predictions[i][f'prediction_winnerR'] = models['model_winnerR'].predict(data['data']['data'][i])
      ...
      confidences[i][f'confidence_winnerR'] = models['model_winnerR'].predict_proba(data['data']['data'][i])
