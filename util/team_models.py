...
from constants.constants import VERSION, FILE_VERSION, XGB_VERSION, XGB_FILE_VERSION, TORCH_FILE_VERSION, RANDOM_STATE, START_SEASON, END_SEASON
...
W_MODELS = {team: load(f'models/nhl_ai_v{XGB_FILE_VERSION}_xgboost_team{team}_winB.joblib') for team in TEAM_IDS}
L_MODELS = {team: load(f'models/nhl_ai_v{XGB_FILE_VERSION}_xgboost_team{team}_lossB.joblib') for team in TEAM_IDS}
...
def PREDICT(data, team, wModels, lModels):
  ...
  w_prediction = [1 if i > 0.5 else 0 for i in w_probability]
  l_prediction = [1 if i < 0.5 else 0 for i in l_probability]
  # print('w_probability',w_probability)
  # print('l_probability',l_probability)
  # print('w_prediction',w_prediction)
  # print('l_prediction',l_prediction)
...
def TEST_H2H(datas, wModels, lModels):
  ...
  for data in datas:
    ...
    lmAway = lModels[data['awayTeam']]
    wmHome = wModels[data['homeTeam']]
    lmHome = lModels[data['homeTeam']]
    ...
    w_probability_away.append(wmAway.predict(daway)[0])
    l_probability_away.append(lmAway.predict(daway)[0])
    w_probability_home.append(wmHome.predict(dhome)[0])
    l_probability_home.append(lmHome.predict(dhome)[0])
  ...
  return predictions,away_prediction,home_prediction,away_probability,home_probability,w_prediction_away,l_prediction_away,w_prediction_home,l_prediction_home,w_probability_away,l_probability_away,w_probability_home,l_probability_home
