...
def goalies(db,pbgs,allPlayers,game,isAway=True,REPLACE_VALUE=-1):
  ...
  return {
    ...
    f'{homeAway}StartingGoalieGamesPlayed': getGamesPlayed(db,startingId,game['id']),
...
def goalie(db,playerId,allPlayers,game,isStarting=True,isAway=True,REPLACE_VALUE=-1):
  ...
  return {
    ...
    f'{homeAway}{startingBackup}GoalieGamesPlayed': getGamesPlayed(db,playerId,game['id']),
  }