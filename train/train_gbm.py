...
def train(inData):
  ...
  target_labels = ['homeScore','awayScore','winner']

  x = data.drop(target_columns,axis=1)
  y_winner = data['winner']
  y_homeScore = data['homeScore']
  y_awayScore = data['awayScore']


  x_train_winner, x_test_winner, y_train_winner, y_test_winner = train_test_split(x, y_winner, test_size=0.2)

  train_data_winner = lgb.Dataset(x_train_winner, label=y_train_winner)
  test_data_winner = lgb.Dataset(x_test_winner, label=y_test_winner, reference=train_data_winner)
  
  x_train_homeScore, x_test_homeScore, y_train_homeScore, y_test_homeScore = train_test_split(x, y_homeScore, test_size=0.2)
  
  train_data_homeScore = lgb.Dataset(x_train_homeScore, label=y_train_homeScore)
  test_data_homeScore = lgb.Dataset(x_test_homeScore, label=y_test_homeScore, reference=train_data_homeScore)
  
  x_train_awayScore, x_test_awayScore, y_train_awayScore, y_test_awayScore = train_test_split(x, y_awayScore, test_size=0.2)
  
  train_data_awayScore = lgb.Dataset(x_train_awayScore, label=y_train_awayScore)
  test_data_awayScore = lgb.Dataset(x_test_awayScore, label=y_test_awayScore, reference=train_data_awayScore)
  ...
  parameters = {
    ...
    'metric': 'binary_logloss', # or 'l2' for regression
    'num_leaves': 31,
    'learning_rate': 0.05,
    'feature_fraction': 0.9
  }

  model_winner = lgb.train(parameters, train_data_winner, valid_sets=[test_data_winner], num_boost_round=100)
  model_homeScore = lgb.train(parameters, train_data_homeScore, valid_sets=[test_data_homeScore], num_boost_round=100)
  model_awayScore = lgb.train(parameters, train_data_awayScore, valid_sets=[test_data_awayScore], num_boost_round=100)

  predicted_winner = model_winner.predict(test_data_winner)
  predicted_homeScore = model_homeScore.predict(test_data_homeScore)
  predicted_awayScore = model_awayScore.predict(test_data_awayScore)

  print('winner', predicted_winner)
  print('homeScore', predicted_homeScore)
  print('awayScore', predicted_awayScore)

  dump(model_winner, f'models/nhl_ai_gbm_winner.joblib')
  dump(model_homeScore, f'models/nhl_ai_gbm_homeScore.joblib')
  dump(model_awayScore, f'models/nhl_ai_gbm_awayScore.joblib')
